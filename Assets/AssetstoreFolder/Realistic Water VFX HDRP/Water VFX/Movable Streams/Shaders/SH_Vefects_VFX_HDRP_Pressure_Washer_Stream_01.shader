...
Shader "Vefects/SH_Vefects_VFX_HDRP_Pressure_Washer_Stream_01"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			PackedVaryingsMeshToPS VertexFunction(AttributesMesh inputMesh )
			{
				...
				outputPackedVaryingsMeshToPS.ase_texcoord6.z = eyeDepth;
						...
						)
			{
				...
				float2 uv_PressureTexture = packedInput.ase_texcoord6.xy * _PressureTexture_ST.xy + _PressureTexture_ST.zw;
				float4 tex2DNode53 = tex2D( _PressureTexture, uv_PressureTexture );
				float2 texCoord244 = packedInput.ase_texcoord6.xy * float2( 1,1 ) + float2( 0,0 );
				...
				float2 texCoord262 = packedInput.ase_texcoord6.xy * float2( 1,1 ) + float2( 0,0 );
				...
				float lerpResult3_g22 = lerp( ( -1.0 * temp_output_4_0_g22 ) , temp_output_4_0_g22 , isFrontFace);
				...
				float eyeDepth = packedInput.ase_texcoord6.z;
				float cameraDepthFade111 = (( eyeDepth -_ProjectionParams.y - _CameraDepthFadeOffset ) / _CameraDepthFadeLength);
				float op62 = saturate( ( saturate( ( saturate( ( saturate( ( ( ( tex2DNode53.r * ( lerpResult250 * tex2D( _PressureTexture, ( float3( ( ( texCoord1_g21 * appendResult4_g21 ) + appendResult5_g21 ) ,  0.0 ) + float3( ( ( ( (tex2D( _PressureDistortionTextureNormal, panner70 ).rgb).xy + -0.5 ) * 2.0 ) * lerpResult266 ) ,  0.0 ) ).xy ).g ) ) * _OpacityMult ) * distanceDepth36 ) ) * ( 1.0 - fresnelNode1_g22 ) ) ) * saturate( cameraDepthFade111 ) ) ) * packedInput.ase_color.a ) );
				...
				float4 temp_output_287_0 = ( fetchOpaqueVal23 * 0.0001 );
				float4 lerpResult24 = lerp( temp_output_287_0 , ( float4( _Color01 , 0.0 ) + temp_output_287_0 ) , _ColorStrength);
		...
		Pass
		{
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				float3 lerpResult235 = lerp( float3(0,0,1) , tex2D( _PressureTextureNormal, panner70 ).rgb , _NormalIntensity);
				float3 norm100 = lerpResult235;
				float2 uv_PressureTexture = packedInput.ase_texcoord3.xy * _PressureTexture_ST.xy + _PressureTexture_ST.zw;
				float4 tex2DNode53 = tex2D( _PressureTexture, uv_PressureTexture );
				float2 texCoord244 = packedInput.ase_texcoord3.xy * float2( 1,1 ) + float2( 0,0 );
				float lerpResult250 = lerp( 1.0 , saturate( texCoord244.y ) , streamLengthPercentage125);
				float2 texCoord1_g21 = packedInput.ase_texcoord3.xy * float2( 1,1 ) + float2( 0,0 );
				float2 appendResult4_g21 = (float2(1.0 , 1.0));
				float2 appendResult5_g21 = (float2(0.0 , 0.0));
				float2 texCoord262 = packedInput.ase_texcoord3.xy * float2( 1,1 ) + float2( 0,0 );
				float lerpResult266 = lerp( _UVNoiseStrength , 0.0 , saturate( ( saturate( texCoord262.y ) - 0.5 ) ));
				float screenDepth36 = LinearEyeDepth(SampleCameraDepth( ase_screenPosNorm.xy ),_ZBufferParams);
				float distanceDepth36 = saturate( ( screenDepth36 - LinearEyeDepth( ase_screenPosNorm.z,_ZBufferParams ) ) / ( _DepthFade ) );
		...
		Pass
		{
			...
			UNITY_SETUP_INSTANCE_ID(packedInput);
				...
				float2 uv_PressureTexture = packedInput.ase_texcoord1.xy * _PressureTexture_ST.xy + _PressureTexture_ST.zw;
				float4 tex2DNode53 = tex2D( _PressureTexture, uv_PressureTexture );
				float2 texCoord244 = packedInput.ase_texcoord1.xy * float2( 1,1 ) + float2( 0,0 );
				float streamLengthPercentage125 = _StreamLengthPercentage;
				float lerpResult250 = lerp( 1.0 , saturate( texCoord244.y ) , streamLengthPercentage125);
				...
				float2 appendResult5_g19 = (float2(0.0 , 0.0));
				float2 panner70 = ( 1.0 * _Time.y * _PanSpeed + ( float3( ( ( texCoord55 * appendResult4_g19 ) + appendResult5_g19 ) ,  0.0 ) + float3( 0,0,0 ) ).xy);
				float2 texCoord262 = packedInput.ase_texcoord1.xy * float2( 1,1 ) + float2( 0,0 );
				float lerpResult266 = lerp( _UVNoiseStrength , 0.0 , saturate( ( saturate( texCoord262.y ) - 0.5 ) ));
				float4 screenPos = packedInput.ase_texcoord2;
				float4 ase_screenPosNorm = screenPos / screenPos.w;
				ase_screenPosNorm.z = ( UNITY_NEAR_CLIP_VALUE >= 0 ) ? ase_screenPosNorm.z : ase_screenPosNorm.z * 0.5 + 0.5;
		...
		Pass
		{
			...
			AttributesMesh ApplyMeshModification(AttributesMesh inputMesh, float3 timeParameters, inout PackedVaryingsMeshToPS outputPackedVaryingsMeshToPS )
			{
				...
				float3 objectToViewPos = TransformWorldToView(TransformObjectToWorld(inputMesh.positionOS));
				float eyeDepth = -objectToViewPos.z;
				outputPackedVaryingsMeshToPS.ase_texcoord8.z = eyeDepth;
						...
						)
			{
				...
				float3 lerpResult235 = lerp( float3(0,0,1) , tex2D( _PressureTextureNormal, panner70 ).rgb , _NormalIntensity);
				float3 norm100 = lerpResult235;
				float2 uv_PressureTexture = packedInput.ase_texcoord8.xy * _PressureTexture_ST.xy + _PressureTexture_ST.zw;
				float4 tex2DNode53 = tex2D( _PressureTexture, uv_PressureTexture );
				float2 texCoord244 = packedInput.ase_texcoord8.xy * float2( 1,1 ) + float2( 0,0 );
				float lerpResult250 = lerp( 1.0 , saturate( texCoord244.y ) , streamLengthPercentage125);
				float2 texCoord1_g21 = packedInput.ase_texcoord8.xy * float2( 1,1 ) + float2( 0,0 );
				float2 appendResult4_g21 = (float2(1.0 , 1.0));
				float2 appendResult5_g21 = (float2(0.0 , 0.0));
				float2 texCoord262 = packedInput.ase_texcoord8.xy * float2( 1,1 ) + float2( 0,0 );
				float lerpResult266 = lerp( _UVNoiseStrength , 0.0 , saturate( ( saturate( texCoord262.y ) - 0.5 ) ));
				float screenDepth36 = LinearEyeDepth(SampleCameraDepth( ase_screenPosNorm.xy ),_ZBufferParams);
				float distanceDepth36 = saturate( ( screenDepth36 - LinearEyeDepth( ase_screenPosNorm.z,_ZBufferParams ) ) / ( _DepthFade ) );
				float3 ase_worldPos = GetAbsolutePositionWS( positionRWS );
				...
				float fresnelNdotV1_g22 = dot( float3(dot(tanToWorld0,tanNormal1_g22), dot(tanToWorld1,tanNormal1_g22), dot(tanToWorld2,tanNormal1_g22)), ase_viewDirWS );
				float fresnelNode1_g22 = ( _FresnelBias + _FresnelScale * pow( 1.0 - fresnelNdotV1_g22, lerpResult3_g22 ) );
				float eyeDepth = packedInput.ase_texcoord8.z;
				float cameraDepthFade111 = (( eyeDepth -_ProjectionParams.y - _CameraDepthFadeOffset ) / _CameraDepthFadeLength);
				float op62 = saturate( ( saturate( ( saturate( ( saturate( ( ( ( tex2DNode53.r * ( lerpResult250 * tex2D( _PressureTexture, ( float3( ( ( texCoord1_g21 * appendResult4_g21 ) + appendResult5_g21 ) ,  0.0 ) + float3( ( ( ( (tex2D( _PressureDistortionTextureNormal, panner70 ).rgb).xy + -0.5 ) * 2.0 ) * lerpResult266 ) ,  0.0 ) ).xy ).g ) ) * _OpacityMult ) * distanceDepth36 ) ) * ( 1.0 - fresnelNode1_g22 ) ) ) * saturate( cameraDepthFade111 ) ) ) * packedInput.ase_color.a ) );
				float4 fetchOpaqueVal23 = ASEHDSampleSceneColor(( (ase_screenPosNorm).xy + ( ( ( (norm100).xy + -0.5 ) * 2.0 ) * ( ( _NormalDistortionIntensity * streamLengthPercentage125 ) * op62 ) ) ), 0.0, GetInverseCurrentExposureMultiplier());
				float4 temp_output_287_0 = ( fetchOpaqueVal23 * 0.0001 );
		...
		Pass
        {
			...
			UNITY_SETUP_INSTANCE_ID(packedInput);
				...
				float2 uv_PressureTexture = packedInput.ase_texcoord7.xy * _PressureTexture_ST.xy + _PressureTexture_ST.zw;
				float4 tex2DNode53 = tex2D( _PressureTexture, uv_PressureTexture );
				float2 texCoord244 = packedInput.ase_texcoord7.xy * float2( 1,1 ) + float2( 0,0 );
				float streamLengthPercentage125 = _StreamLengthPercentage;
				float lerpResult250 = lerp( 1.0 , saturate( texCoord244.y ) , streamLengthPercentage125);
				...
				float2 appendResult5_g19 = (float2(0.0 , 0.0));
				float2 panner70 = ( 1.0 * _Time.y * _PanSpeed + ( float3( ( ( texCoord55 * appendResult4_g19 ) + appendResult5_g19 ) ,  0.0 ) + float3( 0,0,0 ) ).xy);
				float2 texCoord262 = packedInput.ase_texcoord7.xy * float2( 1,1 ) + float2( 0,0 );
				float lerpResult266 = lerp( _UVNoiseStrength , 0.0 , saturate( ( saturate( texCoord262.y ) - 0.5 ) ));
...
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;287;-1024,0;Inherit;False;2;2;0;COLOR;0,0,0,0;False;1;FLOAT;0;False;1;COLOR;0
Node;AmplifyShaderEditor.RangedFloatNode;288;-1024,128;Inherit;False;Constant;_Float0;Float 0;31;0;Create;True;0;0;0;False;0;False;0.0001;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.TemplateMultiPassMasterNode;276;0,0;Float;False;True;-1;2;Rendering.HighDefinition.LightingShaderGraphGUI;0;14;Vefects/SH_Vefects_VFX_HDRP_Pressure_Washer_Stream_01;53b46d85872c5b24c8f4f0a1c3fe4c87;True;GBuffer;0;0;GBuffer;34;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;True;3;RenderPipeline=HDRenderPipeline;RenderType=Transparent=RenderType;Queue=Transparent=Queue=0;True;5;True;7;d3d11;metal;vulkan;xboxone;xboxseries;playstation;switch;0;False;False;False;False;False;False;False;False;False;False;False;False;False;False;True;0;True;_CullMode;False;False;False;False;False;False;False;False;False;True;True;0;True;_StencilRefGBuffer;255;False;;255;True;_StencilWriteMaskGBuffer;7;False;;3;False;;0;False;;0;False;;7;False;;3;False;;0;False;;0;False;;False;False;True;0;True;_ZTestGBuffer;False;True;1;LightMode=GBuffer;False;False;0;;0;0;Standard;38;Surface Type;1;638800704327700086;  Rendering Pass;1;0;  Refraction Model;0;0;    Blending Mode;0;0;    Blend Preserves Specular;0;638800704338927991;  Back Then Front Rendering;0;0;  Transparent Depth Prepass;0;0;  Transparent Depth Postpass;0;0;  ZWrite;0;0;  Z Test;4;0;Double-Sided;1;638800704375696290;Alpha Clipping;0;0;  Use Shadow Threshold;0;0;Material Type,InvertActionOnDeselection;4;638800704361291230;  Energy Conserving Specular;0;638800704387369644;  Transmission,InvertActionOnDeselection;0;0;Receive Decals;0;638800704384733440;Receive SSR;1;0;Receive SSR Transparent;0;0;Motion Vectors;1;0;  Add Precomputed Velocity;0;0;Specular AA;0;0;Specular Occlusion Mode;1;0;Override Baked GI;0;0;Depth Offset;0;0;  Conserative;1;0;GPU Instancing;1;0;LOD CrossFade;0;0;Tessellation;0;0;  Phong;0;0;  Strength;0.5,False,;0;  Type;0;0;  Tess;16,False,;0;  Min;10,False,;0;  Max;25,False,;0;  Edge Length;16,False,;0;  Max Displacement;25,False,;0;Vertex Position;1;0;0;11;True;True;True;True;True;True;False;False;False;True;True;False;;False;0
...
WireConnection;287;0;23;0
WireConnection;287;1;288;0
WireConnection;276;0;24;0
WireConnection;276;1;101;0
WireConnection;276;6;102;0
WireConnection;276;7;105;0
WireConnection;276;9;63;0
ASEEND*/
//CHKSM=0CA7DE0F15BF2BE61419B36F32BB063FFE501D46